		...
		<div class="tweets wf-container" id="wf-container">
			<template v-for="entity in items">
				<template v-for="handle in entity">
					...
					<!--TWITTER-->
					<twitter :handle=handle v-if="handle.social_media === 'twitter' && activeSocialMedia.findIndex('twitter')!==-1"></twitter>

					<!--YOUTUBE-->
					<youtube :handle=handle v-if="handle.social_media === 'youtube' && activeSocialMedia.findIndex('youtube')!==-1"></youtube>

					<!--VIMEO-->
					<vimeo :handle=handle v-if="handle.social_media === 'vimeo' && activeSocialMedia.findIndex('vimeo')!==-1"></vimeo>
